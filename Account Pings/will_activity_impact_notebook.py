...
dbutils.widgets.text("owner_email", "will.schweitzer@databricks.com")
dbutils.widgets.text("fiscal_year", "FY27")
...
WITH activity_base AS (
  ...
  WHERE lower(csm.BDR_PSM_CSM) = '{OWNER_EMAIL}'
    ...
    AND (
      '{FISCAL_YEAR}' = ''
      OR upper(CAST(csm.fy_year AS STRING)) = upper('{FISCAL_YEAR}')
      OR upper(csm.fiscalQuarter) LIKE concat('%', upper('{FISCAL_YEAR}'), '%')
    )
),
activity_rows AS (
  SELECT
...
impact_csv = f"{out_dir}/will_activity_impact_all_touches_{RUN_DATE_STR}.csv"
impact_md = f"{out_dir}/will_activity_impact_all_touches_{RUN_DATE_STR}.md"
first_csv = f"{out_dir}/will_activity_impact_first_touch_{RUN_DATE_STR}.csv"
first_md = f"{out_dir}/will_activity_impact_first_touch_{RUN_DATE_STR}.md"
