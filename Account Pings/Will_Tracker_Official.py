...
for segment_focus in ["strategic", "named"]:
    ...
    caio_flags AS (
      SELECT
        ...
        MAX(TRY_TO_DATE(TRIM(date), 'M/d/yyyy')) AS last_caio_meeting_date
    ...
    if not result.empty:
        ...
        result["oversight_match_score"] = pd.Series(dtype=float)
        result["is_oversight_account"] = pd.Series(dtype=bool)
    ...
    if result.empty:
        result["top_ai_sku"] = pd.Series(dtype=str)
        result["top_ai_sku_delta"] = pd.Series(dtype=float)
...
try:
    ...
    plt = None
    print(f"Visualization skipped: matplotlib unavailable ({_plot_err})")

if plt is not None and "strategic" in segment_exports and not segment_exports["strategic"].empty:
    strategic_for_viz = segment_exports["strategic"].copy()
    growers_viz = (
        strategic_for_viz[strategic_for_viz["signal_type"] == "grower"]
    ...
    decliners_viz = (
        strategic_for_viz[strategic_for_viz["signal_type"] == "decliner"]
        .sort_values(["pop_growth_pct", "total_t28d_spend"], ascending=[True, False])
        .drop_duplicates(subset=["account_id"], keep="first")
        .head(10)
        .copy()
    )
    growers_viz["viz_group"] = "Grower"
    decliners_viz["viz_group"] = "Decliner"
    viz_accounts = pd.concat([growers_viz, decliners_viz], ignore_index=True)

    if viz_accounts.empty:
        print("Visualization skipped: no Strategic growers/decliners found.")
    else:
        viz_ids = sorted(viz_accounts["account_id"].astype(str).unique().tolist())
        viz_id_sql = ", ".join(["'" + x.replace("'", "''") + "'" for x in viz_ids])
        viz_sql = f"""
        SELECT
          account_id,
          account_name,
          usage_date AS date,
          SUM(COALESCE(genai_gpu_model_serving_dbu_dollars,0)) AS gpu_model_serving_dollars,
          SUM(COALESCE(genai_throughput_model_serving_dbu_dollars,0)) AS throughput_serving_dollars,
          SUM(COALESCE(genai_foundation_model_api_dbu_dollars,0)) AS foundational_model_api_dollars,
          SUM(COALESCE(genai_vector_search_dbu_dollars,0)) AS vector_search_dollars,
          SUM(COALESCE(genai_cpu_model_serving_dbu_dollars,0)) AS cpu_model_serving_dollars,
          SUM(COALESCE(genai_foundation_model_training_dbu_dollars,0)) AS foundational_model_training_dollars,
          SUM(COALESCE(batch_interference_model_serving_dbu_dollars,0)) AS batch_inference_model_serving_dollars,
          SUM(COALESCE(agent_evals_dbu_dollars,0)) AS agent_eval_dollars,
          SUM(COALESCE(anthropic_dbu_dollars,0)) AS anthropic_dollars,
          SUM(COALESCE(ai_gateway_dbu_dollars,0)) AS ai_gateway_dollars,
          SUM(COALESCE(ai_runtime_dbu_dollars,0)) AS ai_runtime_dollars,
          SUM(COALESCE(genai_mct_dbu_dollars,0) + COALESCE(ai_dbu_dollars,0)) AS ai_dollars
        FROM main.gtm_gold.account_consumption_daily
        WHERE account_id IN ({viz_id_sql})
          AND usage_date BETWEEN date_sub({RUN_DATE_EXPR}, 60) AND date_sub({RUN_DATE_EXPR}, 2)
        GROUP BY account_id, account_name, usage_date
        ORDER BY account_name, usage_date
        """
        viz_daily = spark.sql(viz_sql).toPandas()
        if viz_daily.empty:
            print("Visualization skipped: no daily consumption rows for selected Strategic accounts.")
        else:
            dollar_cols = [
                "gpu_model_serving_dollars",
                "throughput_serving_dollars",
                "foundational_model_api_dollars",
                "vector_search_dollars",
                "cpu_model_serving_dollars",
                "foundational_model_training_dollars",
            ...
            for _, acc in viz_accounts.iterrows():
                ...
                growth = float(acc["pop_growth_pct"]) if pd.notna(acc["pop_growth_pct"]) else 0.0
                t28d = float(acc["total_t28d_spend"]) if pd.notna(acc["total_t28d_spend"]) else 0.0

                pdf = viz_daily[viz_daily["account_id"].astype(str) == account_id].copy()
                if pdf.empty:
                    continue
                pdf["date"] = pd.to_datetime(pdf["date"])
                pdf = pdf.sort_values("date")

                cols_to_plot = [c for c in dollar_cols if c in pdf.columns and pd.to_numeric(pdf[c], errors="coerce").fillna(0).sum() > 0]
                if not cols_to_plot:
                    continue
                ...
                for col in cols_to_plot:
                    ...
                    if bottom is None:
                        plt.bar(pdf["date"], vals, label=col, color=color_map[col], alpha=0.85)
                        bottom = vals.copy()
                    else:
                        plt.bar(pdf["date"], vals, bottom=bottom, label=col, color=color_map[col], alpha=0.85)
                        bottom = bottom + vals

                plt.plot(pdf["date"], pd.to_numeric(pdf["ai_dollars"], errors="coerce").fillna(0), color="black", linewidth=1.8, label="total_ai_dollars")
                plt.title(f"{signal.title()} | {account_name} | T28D=${t28d:,.2f} | PoP={growth:+.2f}%")
                plt.xlabel("Date")
                plt.ylabel("AI Dollars")
...
if "strategic" in segment_exports and not segment_exports["strategic"].empty:
    strategic_df = segment_exports["strategic"].copy()
    strategic_df["abs_growth"] = strategic_df["pop_growth_pct"].abs()
    top20_strategic = (
        strategic_df.sort_values(["abs_growth", "total_t28d_spend"], ascending=[False, False])
        .drop_duplicates(subset=["account_id"], keep="first")
        .head(20)
        .copy()
    )

    if not top20_strategic.empty:
        top20_ids = sorted(top20_strategic["account_id"].astype(str).unique().tolist())
        id_list_sql = ", ".join(["'" + x.replace("'", "''") + "'" for x in top20_ids])
        ...
        WHERE account_id IN ({id_list_sql})
          AND (
            use_case_product = 'AI'
            OR use_case_product LIKE 'AI;%'
            OR use_case_product LIKE '%;AI;%'
            OR use_case_product LIKE '%;AI'
            OR use_case_product LIKE '%GenAI%'
          )
          AND stage IN ('U1','U2','U3','U4','U5')
        """

        open_usecases = spark.sql(usecase_sql).toPandas()
        top20_meta_cols = [
            "account_id",
            "account_name",
            "signal_type",
        ...
        top20_meta = top20_strategic[top20_meta_cols].drop_duplicates(subset=["account_id"])

        if open_usecases.empty:
            ...
            strategic_open_uc = top20_meta.merge(open_usecases, on="account_id", how="left")
            stage_rank = {"U5": 5, "U4": 4, "U3": 3, "U2": 2, "U1": 1}
            strategic_open_uc["stage_rank"] = strategic_open_uc["stage"].map(stage_rank).fillna(0)
            strategic_open_uc = strategic_open_uc.sort_values(
                ["total_t28d_spend", "pop_growth_pct", "stage_rank", "estimated_monthly_dollar_dbus"],
                ascending=[False, False, False, False],
            )
            strategic_open_uc = strategic_open_uc.drop(columns=["stage_rank"])

        strategic_open_csv_uri = f"{weekly_dir}/strategic_top20_open_usecases_{run_date}.csv"
        strategic_open_md_uri = f"{weekly_dir}/strategic_top20_open_usecases_{run_date}.md"
        dbutils.fs.put(strategic_open_csv_uri, strategic_open_uc.to_csv(index=False), True)

        strategic_open_md = [
            f"# Strategic Top 20 Accounts - Open AI Use Cases ({run_date})",
            "",
            "- Accounts selected: top 20 Strategic accounts by absolute T28D growth signal",
            "- Open use cases: AI/GenAI products in U1-U5",
            "",
        ]
        if strategic_open_uc.empty:
            strategic_open_md.append("No open AI use cases found for these accounts.")
        else:
            md_view_cols = [
                "account_name",
                "signal_type",
            ...
            strategic_open_md.append(to_markdown_table(strategic_open_uc[md_view_cols]))
        dbutils.fs.put(strategic_open_md_uri, "\n".join(strategic_open_md).strip() + "\n", True)

        print(f"Wrote Strategic Open Use Cases CSV: {strategic_open_csv_uri}")
        print(f"Wrote Strategic Open Use Cases MD: {strategic_open_md_uri}")
        if not strategic_open_uc.empty:
            spark.createDataFrame(strategic_open_uc).display()
    else:
        print("Strategic top-20 use case table skipped: no Strategic signal accounts found.")
else:
